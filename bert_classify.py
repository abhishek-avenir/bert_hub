...
def train(tokenizer, do_eval=True):
	print("[INFO] Loading data from folders...")
...
def predict(tokenizer, predict_file=None, text=None):
...
def main(mode, skip_eval=False, predict_file=None, text=None):
	...
	if mode == 'train':
		train(tokenizer, do_eval=(not skip_eval))
	else:
		predict(tokenizer, predict_file, text)
